...
# LOAD DATA
a <- data.frame(fread("J:\\data_and_prep_utilities\\59070_application\\accelerometer-data\\sep20-summary-all.csv")) # all acc data
...
for (label in list(c("MVPA", "Probability in MVPA"),  c("light", "Probability in LIPA"),  c("sedentary", "Probability in SB"),  c("sleep", "Probability in sleep"))){
  lab <- paste0("p_", label[1])
  assign(lab, plotAverageDay(a, paste0(label[1], ".hourOfDay."), ".avg", yAxisLabel = label[2]))
}

# WRITE OUT FIGURE
svg(paste0("ukbAccProcessing/plots/", name_of_current_run, "_face_validity.svg", sep = ""), width = 6, height = 8 )
